var richTextEditor = {
    ...
    methods: {
        ...
        implementStyle: function (selectedNodeList, parent, style, nextElementSibling) {
            ...
            const classMatches = htmlString.match(/class\s*=\s*"([^"]+)"/g);
        
            let classAttribute = '';
        
            if (classMatches && classMatches.length > 0) {
                const classNames = classMatches
                    .map(match => match.match(/class\s*=\s*"([^"]+)"/)[1]) 
                    .flatMap(className => className.split(' ')); 
                classAttribute = ` class="${classNames.join(' ')}"`;
            }

            console.log(classAttribute,'classatribute');
                if (index == -1 && Object.keys(siblingData).length) {
                    let domType = ['insertUnorderedList', 'insertOrderedList'].includes(style) ? 'li' : 'div';
            
                    if (classAttribute) {
                        let outerHtml = `${siblingData['previousSibling'] || ''}<${domType}><span${classAttribute}>${innertext}</span></${domType}>`;
        ...
        toggleSubBullets: function (selectedNodeList, parent, action) {
            ...
            if (action === 'insert') {
                const classNames = selectedNodeList.map(node => node.className);
                debugger
                ...
                selectedNodeList.forEach((node, index) => {
                    node.className = classNames[index];
                });
                this.onInput(parent, true);
            } else {
        ...
        differentiateStyling: function (style) {
            debugger;
            const richTextEditorField = this.$el.querySelector('#richTextEditor-text');
            ...
            if (richTextEditorField.childNodes.length) {
                ...
                Array.from(richTextEditorField.childNodes).forEach((node) => {
                    if (childsNotSelected || selectedRange.intersectsNode(node)) {
